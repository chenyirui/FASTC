...
class RegSeg(nn.Module):
    ...
    def __init__(self, name, num_classes, pretrained="", ablate_decoder=False,change_num_classes=False,num_input_feature=64):
        super().__init__()
        self.stem=ConvBnAct(num_input_feature,32,3,2,1)
...
def camvid_speed_test():
    ...
    ts.extend(benchmark_eval([regseg,ddrnet23],x,True))
    print(ts)
def fastc_speed_test():
    print("fastc_speed test")
    #from competitors_models.DDRNet_Reimplementation import get_ddrnet_23
    regseg=RegSeg("exp48_decoder26",num_classes=5,num_input_feature=128).cuda()
    regseg=regseg.eval()
    #model1=RegSeg("exp48_decoder26",19).eval()
    #ddrnet23=get_ddrnet_23()
    ...
    for _ in range(100):
        out = regseg(x)
        #logits = model.predict(scan, pose)[0]
    torch.cuda.synchronize()
    torch.cuda.synchronize()
    elapsed_time = time.time() - t_start
    latency = elapsed_time / 100 * 1000
    FPS = 1000 / latency
    print(latency)
    #ts=[]
    #ts.extend(benchmark_eval([regseg],x,True))
    #print(ts)    
